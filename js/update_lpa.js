...
var div_val = '<div class="ui-grid-c vgb" id="up_checkbox'+c+'"><div class="ui-block-a "><label class="flo">Yes:</label><input type="radio" id="up_result_yes_a'+c+'" name="up_result_yes'+c+'"  value="yes" ></div><div class="ui-block-b"><label class="flo">No:</label><input type="radio" id="up_result_yes_b'+c+'" name="up_result_yes'+c+'" value="no" class="marl"><br id="up_result_no_br'+c+'"></div><div class="ui-block-c "><label class="flo">N/A:</label ><input type="radio" id="up_result_yes_c'+c+'" name="up_result_yes'+c+'" value="na"></div><div class="ui-block-d"><a href="#" class="ui-btn pls ui-shadow ui-corner-all ui-icon-plus ui-btn-icon-notext"  id="up_add'+c+'" onclick="showfields('+c+');">up_add</a><a href="#" id="up_minus'+c+'" class="ui-btn pls ui-shadow ui-corner-all ui-icon-minus ui-btn-icon-notext" style="display:none" onclick="hidefields('+c+');" >up_minus</a><br id="up_up_result_na_br'+c+'" style="display:none;"></div></div><div class="paf"><input type="text"   id="up_findings'+c+'" name="up_findings'+c+'" value="'+activityobj.findings+'" placeholder="Findings" style="display:none;"><select name="up_responsibility'+c+'" id="up_responsibility'+c+'" ><option value="0">Choose Responsibility</option><option value="supervisor">Supervisor</option><option value="Manager">Manager</option><option value="GM">GM</option></select><input type="date" style="display:none;"  id="up_datepicker'+c+'" name="up_datepicker'+c+'" value="'+activityobj.closed_date+'"/></div>';
		
		

				//alert('activityobj.evaluation_status' + activityobj.evaluation_status + 'up_responsibility' +activityobj.reponsible_person + activityobj.closed_date );
   					$('#update_pagination').append(act+div_val);
					$('#update_pagination').enhanceWithin();
				//	$('#up_result_yes'+c).val(activityobj.evaluation_status).checkboxradio("refresh");
				...
				$('#up_responsibility'+c).val(activityobj.reponsible_person).attr("selected",true).selectmenu("refresh", true);
					...
					//$('up_findings'+c).val(activityobj.findings);
					//$('up_datepicker'+c).val(activityobj.closed_date);
					c++;
			...
			if((up_total+1)%5>0){
				up_totalpage = Math.floor((up_total+1)/5)+1;
			}else {
				up_totalpage = Math.floor((up_total+1)/5);
			}
		...
		function pagination2(){
					...
					if(up_currentpage>1 && up_currentpage<up_totalpage){
						start = (up_currentpage-1)*5;
						end =((up_currentpage)*5)-1;
